...
bool
DeviceManagement::OnSetSystemOptions( const QJsonObject& params )
{
...
bool
DeviceManagement::OnStartupIndication( const SerialMessage& serialMsg, QJsonObject& result )
{
    // check minimum payload length
...
bool
DeviceManagement::OnDeviceInfoResponse( const SerialMessage& serialMsg, QJsonObject& result )
{
 ...
 *
 * @return  QJsonObject including decoded data
 */
...
DeviceManagement::DecodeDeviceInfo( const SerialMessage& serialMsg, int index ) const
{
    ...
    info[ "Module Type" ]   =   _ModuleTypes.value( moduleType, "unknown module type:" + QString::number( moduleType ) );
    info[ "Module ID" ]     =   QString::number( serialMsg.GetU32( index + 1 ) );
...
bool
DeviceManagement::OnFirmwareVersionResponse( const SerialMessage& serialMsg, QJsonObject& result )
{
    // check minimum payload length
...
{
    QJsonObject info;
    info[ "Version" ]           =   QString::number( serialMsg.GetU8( index + 1 ) ) + "." + QString::number( serialMsg.GetU8( index ) );
    info[ "Build Count" ]       =   QString::number( serialMsg.GetU16( index + 2 ) );
    info[ "Build Date" ]        =   QString( serialMsg.GetPayload( index + 4, 10 ) );
    info[ "Firmware Name" ]     =   QString( serialMsg.GetPayload( index + 14 ) );
...
bool
DeviceManagement::OnDateTimeResponse( const SerialMessage& serialMsg, QJsonObject& result )
{
    // check minimum payload length
    ...
    {
        QJsonObject info;
        info[ "Seconds since epoch" ]   =   QString::number( serialMsg.GetU32( SerialMessage::ResponseData_Index ) );
...
bool
DeviceManagement::OnSystemOptionsResponse( const SerialMessage& serialMsg, QJsonObject& result )
{
    // check minimum payload length
    if ( serialMsg.GetResponsePayloadLength() < ( 4 ) )
        return false;
    ...
    if ( status == Ok )
    {
        ...
        QJsonObject info;
        info[ "Options.APS" ]           =   ( options & SO_APS )            ? "on" : "off";
        info[ "Options.Trace" ]         =   ( options & SO_Trace )          ? "on" : "off";
        info[ "Options.RTC" ]           =   ( options & SO_RTC )            ? "on" : "off";
        info[ "Options.WatchDog" ]      =   ( options & SO_WatchDog )       ? "on" : "off";
        info[ "Options.Startup Event" ] =   ( options & SO_StartupEvent )   ? "on" : "off";

        result[ "System Options" ]      =   info;
    }
    return true;
}
